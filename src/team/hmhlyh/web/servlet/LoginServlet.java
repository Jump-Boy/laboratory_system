...
//		if (json.get("state") == null) {
		...
		if (roleFlag.equals("manager")) {
			...
			try {
				...
				//记录访问者信息
				visitRecordService.addVisitRecord(visitorIP, mana.getId(),"manager", "login");
			} catch (ManaException me) {
			...
			try {
				...
				visitRecordService.addVisitRecord(visitorIP, tea.getId(),"teacher", "login");
			} catch (TeaException te) {
			...
			try {
				...
				visitRecordService.addVisitRecord(visitorIP, stu.getId(),"student", "login");
			} catch (StuException se) {
