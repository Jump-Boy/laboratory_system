...
public class ScoreDaoImpl implements ScoreDao {
	...
	public List<Map<String, Object>> findHavenCourse(long teaId, String week,
		...
		List<Map<String, Object>> list = qr.query(ManagerThreadLoacl.getConnection(), "SELECT stuId,courseId FROM (SELECT * FROM course NATURAL JOIN courseTime WHERE (? BETWEEN startWeeks AND endWeeks) AND WEEK=? AND (CURTIME() " +
				"BETWEEN startTime AND endTime) AND teaId=?)AS course,stuselectcourse WHERE id=courseId", new MapListHandler(), currentWeeks,week,teaId);
		return list;
	}
	...
	public Map<String, Object> findHavenUsualScore(long stuId,
		...
		QueryRunner qr = new QueryRunner();
		Map<String, Object> map = qr.query(ManagerThreadLoacl.getConnection(), "SELECT * FROM stuusualscore WHERE stuId=? AND courseId=?", new MapHandler(), stuId, courseId);
		return map;
	}
...
//	}
	...
	public List<Map<String, Object>> readForMarkEvery(long teaId, String week,
			 byte currentWeeks) throws SQLException {
		QueryRunner qr = new QueryRunner();
		List<Map<String, Object>> list = qr.query(ManagerThreadLoacl.getConnection(), "SELECT usualScoreId,stuId,NAME AS stuName,courseName,CONCAT(WEEK,' ',TIME_FORMAT(startTime,'%H:%i') ," +
				"'~',TIME_FORMAT(endTime,'%H:%i')) AS classTime,location,currentWeeks,usualScore FROM student,(SELECT * FROM stuusualscore NATURAL JOIN(SELECT " +
				"* FROM (SELECT * FROM course NATURAL JOIN courseTime WHERE (? BETWEEN startWeeks AND endWeeks) AND WEEK=? AND (CURTIME() " +
				"BETWEEN startTime AND endTime) AND teaId=?)AS course,stuselectcourse WHERE id=courseId)AS curCourse )AS markCourse WHERE " +
				"student.id=markCourse.stuId", new MapListHandler(), currentWeeks, week, teaId);
		return list;
...
//	}
	...
	public int getVacateNum(int courseNo, long stuId) throws SQLException {
		QueryRunner qr = new QueryRunner();
		long vacateNum = (Long) qr.query(ManagerThreadLoacl.getConnection(), "SELECT COUNT(vacate) FROM stuusualScore WHERE vacate='true' AND courseId IN (SELECT id FROM courseTime " +
				"WHERE courseNo=?) AND stuId=?", new ScalarHandler(), courseNo, stuId);
	...
	public int getTruancy(int courseNo, long stuId) throws SQLException {
		QueryRunner qr = new QueryRunner();
		long truancyNum = (Long) qr.query(ManagerThreadLoacl.getConnection(), "SELECT COUNT(truancy) FROM stuusualScore WHERE truancy='true' AND courseId IN (SELECT id FROM courseTime " +
				"WHERE courseNo=?) AND stuId=?", new ScalarHandler(), courseNo, stuId);
	...
	public int getReferScore(int courseNo, long stuId) throws SQLException {
		QueryRunner qr = new QueryRunner();
		int referScore = 0;
		Object obj =  qr.query(ManagerThreadLoacl.getConnection(), "SELECT AVG(usualScore)AS referScore FROM stuusualScore WHERE courseId IN (SELECT id FROM courseTime " +
				"WHERE courseNo=?) AND stuId=?", new ScalarHandler(), courseNo, stuId);
	...
	public List<Map<String, Object>> readForMarkTotalByTea(long teaId)
			throws SQLException {
		QueryRunner qr = new QueryRunner();
		List<Map<String, Object>> scoreList = qr.query(ManagerThreadLoacl.getConnection(), "SELECT totalScoreId, LEFT(grade,2) AS grade, NAME AS  stuName, stuId, className, scoreTable.courseNo,course.courseName, totalScore " +
				"FROM (SELECT * FROM student,stuTotalScore WHERE stuId=id)AS scoreTable,course WHERE course.courseNo=scoreTable.courseNo AND MONTH(NOW())-MONTH(course.createdAt)<6 AND teaId=? ORDER BY grade,stuId", 
	...
	public List<Map<String, Object>> readGroupForMarkTotalByTea(long teaId, String groupGrade)
			throws SQLException {
		QueryRunner qr = new QueryRunner();
		List<Map<String, Object>> scoreList = qr.query(ManagerThreadLoacl.getConnection(), "SELECT totalScoreId,  LEFT(grade,2) AS grade, NAME AS  stuName, stuId, className, scoreTable.courseNo,course.courseName, totalScore FROM (SELECT * FROM " +
				"student,stuTotalScore WHERE stuId=id)AS scoreTable,course WHERE course.courseNo=scoreTable.courseNo AND MONTH(NOW())-MONTH(course.createdAt)<6 AND teaId=? AND LEFT(grade,2)=? ORDER BY grade,stuId", 
	...
	public int isRebuild(int totalScoreId) throws SQLException {
		QueryRunner qr = new QueryRunner();
		int courseNo = 0;
				Object obj = qr.query(ManagerThreadLoacl.getConnection(), "SELECT courseNo FROM course WHERE (SELECT courseName FROM course WHERE courseNo=(SELECT courseNo FROM stutotalscore WHERE totalScoreId=?)) IN (SELECT courseName FROM course WHERE " +
				"courseNo != (SELECT courseNo FROM stutotalscore WHERE totalScoreId=?))", new ScalarHandler(), totalScoreId,totalScoreId);
				if (obj != null) {
	...
	public List<Map<String, Object>> lookGroupScoreTableByStu(long stuId,
		...
		QueryRunner qr = new QueryRunner();
		List<Map<String, Object>> scoreTable = qr.query(ManagerThreadLoacl.getConnection(), "SELECT totalScoreId,courseName,attribute,totalScore,pass,getCredit,GPA,isRebuild FROM course NATURAL JOIN(SELECT * FROM stutotalscore WHERE stuId=?)AS totalScoreTable " +
				"WHERE LOCATE(?,studySemester)", new MapListHandler(), stuId, studySemester);
		return scoreTable;
	}
	...
	public List<Map<String, Object>> lookGroupScoreTable(String studySemester)
			throws SQLException {
		QueryRunner qr = new QueryRunner();
		List<Map<String, Object>> scoreTable = qr.query(ManagerThreadLoacl.getConnection(), "SELECT totalScoreId,courseName,attribute,totalScore,pass,getCredit,GPA,isRebuild FROM course NATURAL JOIN(SELECT * FROM stutotalscore)AS totalScoreTable " +
				"WHERE LOCATE(?,studySemester)", new MapListHandler(), studySemester);
		return scoreTable;
	}
	...
	public List<Map<String, Object>> lookScoreDetail(int totalScoreId)
			throws SQLException {
		QueryRunner qr = new QueryRunner();
		List<Map<String, Object>> scoreDetail = qr.query(ManagerThreadLoacl.getConnection(), "SELECT courseName,classTime,location,currentWeeks,teacher.name AS teaName,usualScore FROM teacher,(SELECT * FROM course NATURAL JOIN(SELECT * FROM " +
				"stuusualscore NATURAL JOIN(SELECT * FROM stuselectcourse NATURAL JOIN(SELECT courseTime.courseNo,stuId,id,CONCAT(WEEK,' ',TIME_FORMAT(startTime,'%H:%i') ,'~',TIME_FORMAT(endTime,'%H:%i'))AS classTime FROM stutotalscore,courseTime " +
				"WHERE totalScoreId=? AND courseTime.courseNo=stutotalscore.courseNo)AS a WHERE a.id=stuselectcourse.courseId)AS b)AS c)AS d WHERE d.teaId=teacher.id;", new MapListHandler(), totalScoreId);
		return scoreDetail;
	}
	...
	public List<Map<String, Object>> readForMarkTotalByMana()
			throws SQLException {
		QueryRunner qr = new QueryRunner();
		List<Map<String, Object>> scoreList = qr.query(ManagerThreadLoacl.getConnection(), "SELECT totalScoreId,  SUBSTRING_INDEX(grade,'第',1) AS grade, NAME AS  stuName, stuId, className, scoreTable.courseNo,course.courseName, totalScore,teaId " +
				"FROM (SELECT * FROM student,stuTotalScore WHERE stuId=id)AS scoreTable,course WHERE course.courseNo=scoreTable.courseNo AND MONTH(NOW())-MONTH(course.createdAt)<6 ORDER BY grade,stuId", 
				new MapListHandler());
		return scoreList;
	}
	...
	public List<Map<String, Object>> readGroupForMarkTotalByMana(
		...
		QueryRunner qr = new QueryRunner();
		List<Map<String, Object>> scoreList = qr.query(ManagerThreadLoacl.getConnection(), "SELECT totalScoreId,  LEFT(grade,2) AS grade, NAME AS  stuName, stuId, className, scoreTable.courseNo,course.courseName, totalScore,teaId FROM (SELECT * FROM " +
				"student,stuTotalScore WHERE stuId=id)AS scoreTable,course WHERE course.courseNo=scoreTable.courseNo AND MONTH(NOW())-MONTH(course.createdAt)<6 AND LEFT(grade,2)=? ORDER BY grade,stuId", 
	...
	public String[] readCourseNameByTotal(int courseNo, long stuId,
		...
		QueryRunner qr = new QueryRunner();
		List<Object> list = qr.query(ManagerThreadLoacl.getConnection(), "SELECT courseName FROM course WHERE courseNo IN (SELECT courseNo FROM stuselectcourse NATURAL JOIN (SELECT courseNo,id AS courseId FROM courseTime WHERE courseNo IN (SELECT courseNo FROM " +
				"course WHERE courseNo!=? AND createdAt<?))AS a WHERE stuId=?)", new ColumnListHandler(), courseNo,createdAt, stuId);
